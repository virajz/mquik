<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Cancel Reasons</flux:heading>
            <flux:text class="mt-1">Why an appointment or pickup/drop job was cancelled — shared by CRM reports and advisor follow-ups.</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('cancel_reason_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('cancel_reason_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'CancelReasonMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('cancel_reason_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'CancelReasonMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'CancelReasonMaster'" wire:key="export-cancel-reasons" />
    <livewire:import-export.import-wizard :module="'CancelReasonMaster'" wire:key="import-cancel-reasons" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('cancel_reason_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('cancel_reason_master.delete')
                                <flux:modal.trigger :name="'cancel-reason-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'cancel-reason-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any appointments are using this cancel reason the delete will fail and you'll see a warning.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('cancel-reason-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No cancel reasons yet</div>
    ...
    <livewire:cancel-reason-master.form />
</div>
