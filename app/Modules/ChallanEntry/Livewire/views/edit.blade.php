...
<div class="max-w-7xl">
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:link :href="route('challan-entry.index')" variant="ghost" class="text-xs">
                <flux:icon.chevron-left class="inline size-3 -mt-0.5" /> Challan Entries
            </flux:link>
            <flux:heading size="xl" level="1" class="mt-1">{{ $editingId ? 'Challan '.$challan_no : 'New Challan Entry' }}</flux:heading>
        </div>
        @if ($editingId)
            @php($t = $this->totals)
    ...
    <form wire:submit="save" novalidate>
        @if (! $editingId)
            @include('challan-entry::partials.section-details')
            <flux:callout class="mt-6" icon="information-circle" variant="secondary">
                <flux:callout.text>Create the challan first — items, charges and attachments unlock once it exists.</flux:callout.text>
            </flux:callout>
            <div class="flex items-center justify-end gap-2 py-6">
                <flux:button :href="route('challan-entry.index')" variant="ghost" wire:navigate>Cancel</flux:button>
                <flux:button type="submit" variant="primary" icon="check">Create Challan</flux:button>
            </div>
        @else
            <flux:tab.group>
                ...
                <flux:tab.panel name="items" class="pt-6">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <flux:heading size="lg">Challan Items</flux:heading>
                            <flux:text size="sm" class="mt-1 text-zinc-500">Parts received, with rate, discount, tax, material condition and invoice status.</flux:text>
                        </div>
                    ...
                    @if (count($items) === 0)
                        <div class="rounded-md border border-dashed border-zinc-300 dark:border-zinc-700 px-4 py-6 text-center text-sm text-zinc-500">
                            Add the parts received on this challan.
                        </div>
                    @else
                        <div class="space-y-2">
                            @foreach ($items as $i => $item)
                                ...
                                <div wire:key="ch-line-{{ $i }}" class="rounded-lg border border-zinc-200 dark:border-zinc-800 p-3 space-y-2">
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                                        ...
                                        <flux:select wire:model="items.{{ $i }}.material_condition" variant="listbox" size="sm" label="Condition">
                                            @foreach (Challan::materialConditions() as $k => $l)
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-[180px_1fr_auto] gap-2 items-end">
                                        <flux:select wire:model="items.{{ $i }}.invoice_status" variant="listbox" size="sm" label="Invoice">
                                            @foreach (Challan::invoiceStatuses() as $k => $l)
                ...
                <flux:tab.panel name="charges" class="pt-6">
                    <section class="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6 lg:gap-10">
                        <div>
                            ...
                            <flux:text size="sm" class="mt-1 text-zinc-500">Discount scheme, additional charge heads, files and the live total.</flux:text>
                        </div>
                        <div class="space-y-4 min-w-0">
                            <flux:select wire:model="discount_scheme" variant="listbox" clearable label="Discount Scheme" placeholder="None" class="max-w-xs">
                                @foreach (Challan::discountSchemes() as $k => $l)
                            ...
                            @else
                                <div class="space-y-2">
                                    @foreach ($charges as $i => $charge)
                                        <div wire:key="ch-charge-{{ $i }}" class="grid grid-cols-1 md:grid-cols-[200px_140px_1fr_auto] gap-2 items-end p-2 rounded-md border border-zinc-200 dark:border-zinc-800">
            ...
            <div class="flex items-center justify-end gap-2 py-6">
                <flux:button :href="route('challan-entry.index')" variant="ghost" wire:navigate>Close</flux:button>
