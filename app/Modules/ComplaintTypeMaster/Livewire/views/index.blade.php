<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Complaint Types</flux:heading>
            <flux:text class="mt-1">Categories of customer complaints — used by complaint logging, advisor routing, and CRM reports.</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('complaint_type_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('complaint_type_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'ComplaintTypeMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('complaint_type_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'ComplaintTypeMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'ComplaintTypeMaster'" wire:key="export-complaint-types" />
    <livewire:import-export.import-wizard :module="'ComplaintTypeMaster'" wire:key="import-complaint-types" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('complaint_type_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('complaint_type_master.delete')
                                <flux:modal.trigger :name="'complaint-type-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'complaint-type-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any complaints are using this type the delete will fail and you'll see a warning.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('complaint-type-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
    ...
    <livewire:complaint-type-master.form />
</div>
