<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Credit Note Reasons</flux:heading>
            <flux:text class="mt-1">Why a vendor credit or debit note is raised for returned goods.</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('credit_note_reason_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('credit_note_reason_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'CreditNoteReasonMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('credit_note_reason_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'CreditNoteReasonMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'CreditNoteReasonMaster'" wire:key="export-credit-note-reasons" />
    <livewire:import-export.import-wizard :module="'CreditNoteReasonMaster'" wire:key="import-credit-note-reasons" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('credit_note_reason_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('credit_note_reason_master.delete')
                                <flux:modal.trigger :name="'credit-note-reason-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'credit-note-reason-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('credit-note-reason-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No Credit Note Reasons yet</div>
                        <flux:text class="mt-1">Add reasons like Excess Qty, Poor Quality, Damage.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:credit-note-reason-master.form />
</div>
