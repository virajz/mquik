<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Damage Causes</flux:heading>
            <flux:text class="mt-1">Damage cause categories for estimates and insurance claims.</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('damage_cause_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('damage_cause_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'DamageCauseMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('damage_cause_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'DamageCauseMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'DamageCauseMaster'" wire:key="export-damage-causes" />
    <livewire:import-export.import-wizard :module="'DamageCauseMaster'" wire:key="import-damage-causes" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('damage_cause_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('damage_cause_master.delete')
                                <flux:modal.trigger :name="'damage-cause-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'damage-cause-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('damage-cause-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        <flux:icon.bolt class="mx-auto mb-3 size-8 text-zinc-400" />
                        <div class="font-medium">No Damage Causes yet</div>
                        <flux:text class="mt-1">Add causes like Accident Damage, Wear and Tear.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:damage-cause-master.form />
</div>
