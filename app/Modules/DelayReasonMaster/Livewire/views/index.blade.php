<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Delay Reasons</flux:heading>
            <flux:text class="mt-1">Reasons a work order is delayed — tools, power, extra diagnosis.</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('delay_reason_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('delay_reason_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'DelayReasonMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('delay_reason_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'DelayReasonMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'DelayReasonMaster'" wire:key="export-delay-reasons" />
    <livewire:import-export.import-wizard :module="'DelayReasonMaster'" wire:key="import-delay-reasons" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('delay_reason_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('delay_reason_master.delete')
                                <flux:modal.trigger :name="'delay-reason-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'delay-reason-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('delay-reason-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No Delay Reasons yet</div>
                        <flux:text class="mt-1">Add reasons like Tool Not Available.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:delay-reason-master.form />
</div>
