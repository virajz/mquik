<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            ...
            <flux:text class="mt-1">HR departments used across employees and payroll.</flux:text>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('department_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'DepartmentMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('department_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'DepartmentMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'DepartmentMaster'" wire:key="export-departments" />
    <livewire:import-export.import-wizard :module="'DepartmentMaster'" wire:key="import-departments" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('department_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('department_master.delete')
                                <flux:modal.trigger :name="'department-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'department-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any employees are assigned to this department the delete will fail and you'll see a warning.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('department-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <flux:text class="mt-1">Add HR departments like Service, Bodyshop, Tyre, Stores, Accounts.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:department-master.form />
</div>
