<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            ...
            <flux:text class="mt-1">Job titles employees can be assigned to.</flux:text>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('designation_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'DesignationMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('designation_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'DesignationMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'DesignationMaster'" wire:key="export-designations" />
    <livewire:import-export.import-wizard :module="'DesignationMaster'" wire:key="import-designations" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('designation_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('designation_master.delete')
                                <flux:modal.trigger :name="'designation-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'designation-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any employees hold this designation the delete will fail and you'll see a warning.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('designation-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No designations yet</div>
                        <flux:text class="mt-1">Add roles like Mechanical Advisor, Technician, Cashier.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:designation-master.form />
</div>
