...
namespace App\Modules\FinalWorkOrder\Livewire;
...
class Edit extends Component
{
    ...
    public string $status = FinalWorkOrder::STATUS_ASSIGNMENT_PENDING;
    ...
    /** ?int — passed via ?from-job-card=ID for the JobCard → FWO handoff. */
    ...
    public function mount(?FinalWorkOrder $finalWorkOrder = null): void
    {
        if ($finalWorkOrder && $finalWorkOrder->exists) {
            $this->load($finalWorkOrder);
    ...
    protected function load(FinalWorkOrder $order): void
    {
    ...
    protected function rules(): array
    {
        return [
            ...
            'status' => ['required', Rule::in(array_keys(FinalWorkOrder::statuses()))],
            'completion_type' => ['nullable', Rule::in(array_keys(FinalWorkOrder::completionTypes()))],
            ...
            'items.*.result' => ['required', Rule::in(array_keys(FinalWorkOrder::results()))],
    ...
    public function findings()
    {
        ...
        return TechnicianFinding::query()
            ...
            ->where('final_work_order_id', $this->editingId)
            ->orderByDesc('id')
            ->get();
    }
    ...
    {
        $this->authorize($this->editingId ? 'final_work_order.update' : 'final_work_order.create');
        ...
        $existing = $this->editingId ? FinalWorkOrder::find($this->editingId) : null;
        if ($data['status'] === FinalWorkOrder::STATUS_ASSIGNED && (! $existing || ! $existing->assigned_at)) {
            $data['assigned_at'] = now();
        }
        if ($data['status'] === FinalWorkOrder::STATUS_WIP && (! $existing || ! $existing->started_at)) {
            $data['started_at'] = now();
        }
        if ($data['status'] === FinalWorkOrder::STATUS_COMPLETED) {
            $data['ended_at'] = now();
        }
        ...
        $order = DB::transaction(function () use ($data, $items, $pauses, $workScopes, $photos, $isCreate) {
            if ($isCreate) {
                $row = FinalWorkOrder::create($data);
                $this->editingId = $row->id;
                $this->order_no = $row->fresh()->order_no;
            } else {
                $row = FinalWorkOrder::findOrFail($this->editingId);
                $row->update($data);
            }
        ...
        if ($isCreate) {
            return redirect()->route('final-work-order.edit', $order->id);
        }

        return redirect()->route('final-work-order.index');
    }

    /**
     * @param  array<int, array{label: string, result: string, notes?: string|null}>  $rows
     */
    protected function syncItems(FinalWorkOrder $order, array $rows): void
    {
        $keptIds = [];
        ...
        foreach ($rows as $i => $row) {
            ...
            if (isset($this->itemBeforeFiles[$i]) && $this->itemBeforeFiles[$i] instanceof TemporaryUploadedFile) {
                ...
                $beforePath = $this->itemBeforeFiles[$i]->store("final-work-orders/{$order->id}/items", 'public');
            }
            if (isset($this->itemAfterFiles[$i]) && $this->itemAfterFiles[$i] instanceof TemporaryUploadedFile) {
                ...
                $afterPath = $this->itemAfterFiles[$i]->store("final-work-orders/{$order->id}/items", 'public');
            }
     ...
     */
    protected function syncPauses(FinalWorkOrder $order, array $rows): void
    {
        $keptIds = [];
     ...
     */
    protected function syncWorkScopes(FinalWorkOrder $order, array $rows): void
    {
        $keptIds = [];
     ...
     * @param  array<int, array<string, mixed>>  $rows
     */
    protected function syncPhotos(FinalWorkOrder $order, array $rows): void
    {
        $keptIds = [];
