...
class Index extends Component
{
    ...
    {
        $this->authorize('final_work_order.delete');

        FinalWorkOrder::findOrFail($id)->delete();
    ...
    {
        $counts = FinalWorkOrder::query()
            ->selectRaw('status, count(*) as aggregate')
            ->groupBy('status')
            ->pluck('aggregate', 'status');

        return [
            'pending' => (int) ($counts[FinalWorkOrder::STATUS_ASSIGNMENT_PENDING] ?? 0),
            // "Active" spans everything a technician currently holds.
            'active' => (int) ($counts[FinalWorkOrder::STATUS_ASSIGNED] ?? 0)
                + (int) ($counts[FinalWorkOrder::STATUS_WIP] ?? 0)
                + (int) ($counts[FinalWorkOrder::STATUS_ON_HOLD] ?? 0),
            'completed' => (int) ($counts[FinalWorkOrder::STATUS_COMPLETED] ?? 0),
            'cancelled' => (int) ($counts[FinalWorkOrder::STATUS_CANCELLED] ?? 0),
        ];
    }

    /** Stream the Work Order Analysis CSV — per-order time metrics. */
    public function download(): StreamedResponse
    {
        $this->authorize('final_work_order.view');

        $rows = FinalWorkOrder::query()
            ->with(['jobCard:id,job_card_no', 'technician:id,name', 'items:id,final_work_order_id,hours', 'pauses'])
        ...
        $statuses = FinalWorkOrder::statuses();

        return response()->streamDownload(function () use ($rows, $statuses) {
            $out = fopen('php://output', 'w');
            fputcsv($out, ['FWO No', 'Job Card', 'Technician', 'Status', 'Gross Mins', 'Pause Mins', 'Net TAT Mins', 'Item Hours', 'Started', 'Ended']);
    ...
    public function render()
    {
        ...
        return view('final-work-order::index', [
            'rows' => $rows,
            'statuses' => FinalWorkOrder::statuses(),
