    ...
    <form wire:submit="save" class="max-w-7xl">
        <div class="mb-6 flex items-start justify-between gap-4">
            ...
            @if ($editingId)
                ...
                }" size="lg">{{ FinalWorkOrder::statuses()[$status] }}</flux:badge>
            @endif
        </div>
        ...
        @if (! $editingId)
            {{-- LEAN CREATE --}}
            @include('final-work-order::partials.section-details', ['lean' => true])
            ...
            <div class="flex items-center justify-end gap-2 py-6">
                <flux:button :href="route('final-work-order.index')" variant="ghost" wire:navigate>Cancel</flux:button>
        ...
        @else
            ...
            <flux:tab.group>
                ...
                <flux:tab.panel name="details" class="pt-6">
                    @include('final-work-order::partials.section-details', ['lean' => false])
                </flux:tab.panel>
                                ...
                                <div wire:key="item-{{ $i }}" class="space-y-3 p-3 rounded-md border border-zinc-200 dark:border-zinc-800">
                                    <div class="grid grid-cols-1 md:grid-cols-[1fr_140px_100px_auto] gap-2 items-end">
                                        ...
                                        <flux:select wire:model="items.{{ $i }}.result" variant="listbox" size="sm" label="Result">
                                            @foreach (FinalWorkOrder::results() as $key => $label)
                        ...
                        <div class="space-y-4 min-w-0">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <flux:select wire:model="status" variant="listbox" label="Status" required>
                                    @foreach (FinalWorkOrder::statuses() as $key => $label)
                                ...
                                <flux:select wire:model="completion_type" variant="listbox" clearable label="Completion Type" placeholder="On completion…">
                                    @foreach (FinalWorkOrder::completionTypes() as $key => $label)
                                        <flux:select.option :value="$key">{{ $label }}</flux:select.option>
                                    @endforeach
                                </flux:select>
                            </div>
        ...
        @if ($editingId)
            ...
            <div class="flex items-center justify-end gap-2 py-6">
                <flux:button :href="route('final-work-order.index')" variant="ghost" wire:navigate>Close</flux:button>
