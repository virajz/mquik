<div>
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Goods Receive & Verification</flux:heading>
            <flux:text class="mt-1">Receive vendor parts, verify against the PO, QC and allocate storage.</flux:text>
        </div>
        <div class="flex items-center gap-2">
            ...
            @can('goods_receipt.create')
                <flux:button variant="primary" icon="plus" :href="route('goods-receipt.create')" wire:navigate>Receive Goods</flux:button>
            @endcan
        </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-3">
        @foreach ([
            ['label' => "Today's Receipts", 'value' => $kpis['today'], 'icon' => 'inbox-arrow-down', 'filter' => null],
            ['label' => 'Verification Pending', 'value' => $kpis['pending'], 'icon' => 'clock', 'filter' => 'verification_pending'],
            ['label' => 'Rejected Materials', 'value' => $kpis['rejected_materials'], 'icon' => 'exclamation-triangle', 'filter' => null],
        ] as $kpi)
    ...
    <div class="mb-4 flex items-center gap-3 flex-wrap">
        <flux:input wire:model.live.debounce.300ms="search" placeholder="Search GRN / PO…" icon="magnifying-glass" clearable class="max-w-md" />
    ...
    <flux:table>
        <flux:table.columns>
            <flux:table.column class="w-28" sortable :sorted="$sortBy === 'grn_no'" :direction="$sortDirection" wire:click="sort('grn_no')">GRN</flux:table.column>
        ...
        <flux:table.rows>
            @forelse ($rows as $row)
                <flux:table.row :key="$row->id">
                    <flux:table.cell class="font-mono text-xs">{{ $row->grn_no }}</flux:table.cell>
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('goods_receipt.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square" :href="route('goods-receipt.edit', $row)" wire:navigate>Edit</flux:button>
                            @endcan
                            @can('goods_receipt.delete')
                                <flux:modal.trigger :name="'grn-delete-' . $row->id"><flux:button size="sm" variant="ghost" icon="trash" /></flux:modal.trigger>
                                <flux:modal :name="'grn-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->grn_no }}?</flux:heading>
                                        <flux:text>This cannot be undone. Lines and attachments are removed too.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            <flux:modal.close><flux:button variant="ghost">Cancel</flux:button></flux:modal.close>
                                            <flux:button variant="danger" wire:click="delete({{ $row->id }})" x-on:click="$flux.modal('grn-delete-{{ $row->id }}').close()">Delete</flux:button>
                                        </div>
                                    </div>
                                </flux:modal>
                            @endcan
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="5" class="text-center text-zinc-500 py-12">
                        <flux:icon.inbox-arrow-down class="mx-auto mb-3 size-8 text-zinc-400" />
                        <div class="font-medium">No goods receipts yet</div>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
