<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Insurance Policy Types</flux:heading>
            <flux:text class="mt-1">Motor insurance policy types used in document collection.</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('insurance_policy_type_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('insurance_policy_type_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'InsurancePolicyTypeMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('insurance_policy_type_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'InsurancePolicyTypeMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'InsurancePolicyTypeMaster'" wire:key="export-insurance-policy-types" />
    <livewire:import-export.import-wizard :module="'InsurancePolicyTypeMaster'" wire:key="import-insurance-policy-types" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('insurance_policy_type_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('insurance_policy_type_master.delete')
                                <flux:modal.trigger :name="'insurance-policy-type-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'insurance-policy-type-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any records use this policy type the delete will fail.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('insurance-policy-type-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
    ...
    <livewire:insurance-policy-type-master.form />
</div>
