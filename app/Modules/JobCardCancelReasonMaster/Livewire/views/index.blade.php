<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Job Card Cancel Reasons</flux:heading>
            <flux:text class="mt-1">Reasons that may be selected when cancelling an open job card.</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('job_card_cancel_reason_master.create')
    ...
    <livewire:import-export.export-button :module="'JobCardCancelReasonMaster'" wire:key="export-job-card-cancel-reasons" />
    <livewire:import-export.import-wizard :module="'JobCardCancelReasonMaster'" wire:key="import-job-card-cancel-reasons" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('job_card_cancel_reason_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('job_card_cancel_reason_master.delete')
                                <flux:modal.trigger :name="'job-card-cancel-reason-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'job-card-cancel-reason-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any job cards reference this reason the delete will fail and you'll see a warning.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('job-card-cancel-reason-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <flux:text class="mt-1">Add reasons like Customer Backed Out, Estimate Too High, Vehicle Not Available.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:job-card-cancel-reason-master.form />
</div>
