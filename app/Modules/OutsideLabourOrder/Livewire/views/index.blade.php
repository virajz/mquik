<div>
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Outside Labour Orders</flux:heading>
            <flux:text class="mt-1">Outside-vendor work orders — vendor + bay assignment, time tracking, findings and evidence photos.</flux:text>
        </div>
        <div class="flex items-center gap-2">
            @can('outside_labour_order.view')
                <flux:button variant="ghost" icon="arrow-down-tray" wire:click="download">Outside Labour Status Report</flux:button>
            @endcan
            @can('outside_labour_order.create')
                <flux:button variant="primary" icon="plus" :href="route('outside-labour-order.create')" wire:navigate>New Work Order</flux:button>
            @endcan
        </div>
    </div>
    ...
    <div class="mb-4 flex items-center gap-3 flex-wrap">
        <flux:input wire:model.live.debounce.300ms="search" placeholder="Search by OLO no, JC no, or reg no..." icon="magnifying-glass" clearable class="max-w-md" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('outside_labour_order.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square" :href="route('outside-labour-order.edit', $row)" wire:navigate>Edit</flux:button>
                            @endcan
                            @can('outside_labour_order.delete')
                                <flux:modal.trigger :name="'olo-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'olo-delete-' . $row->id">
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger" wire:click="delete({{ $row->id }})" x-on:click="$flux.modal('olo-delete-{{ $row->id }}').close()">Delete</flux:button>
                                        </div>
                                    </div>
                                </flux:modal>
                            @endcan
