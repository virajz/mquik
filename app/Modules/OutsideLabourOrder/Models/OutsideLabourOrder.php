...
use App\Modules\OutsideLabourOrder\Database\Factories\OutsideLabourOrderFactory;
...
class OutsideLabourOrder extends Model
{
    use Auditable;
    use HasFactory;
    use Searchable;
    ...
    protected $table = 'outside_labour_orders';
    ...
    protected static function newFactory(): OutsideLabourOrderFactory
    {
        return OutsideLabourOrderFactory::new();
    }
    ...
    protected static function booted(): void
    {
        static::created(function (self $row) {
            if ($row->order_no === null) {
                $row->forceFill([
                    'order_no' => 'OLO-'.str_pad((string) $row->id, 5, '0', STR_PAD_LEFT),
                ])->saveQuietly();
            }
        });
    ...
    {
        return $this->belongsTo(ServiceSpecialistMaster::class, 'order_type_id');
    }
    ...
    {
        return $this->hasMany(OutsideLabourOrderItem::class, 'outside_labour_order_id')->orderBy('sequence_no');
    }

    public function pauses(): HasMany
    {
        return $this->hasMany(OutsideLabourOrderPause::class, 'outside_labour_order_id')->orderBy('paused_at');
    }

    /** Total paused minutes across all pause/resume rows. */
    public function pauseMinutes(): int
    {
        return (int) $this->pauses->reduce(function (int $carry, OutsideLabourOrderPause $p) {
    ...
    {
        return $this->hasMany(OutsideLabourOrderScope::class, 'outside_labour_order_id')->orderBy('sequence_no');
    }

    /** Order-level photo evidence, separate from the per-item before/after pair. */
    public function photos(): HasMany
    {
        return $this->hasMany(OutsideLabourOrderPhoto::class, 'outside_labour_order_id')->orderBy('sequence_no');
    }
    ...
    {
        return $this->hasMany(TechnicianFinding::class, 'outside_labour_order_id');
    }
