...
use App\Modules\OutsideLabourReturn\Models\OutsideLabourReturnAttachment;
use App\Modules\OutsideLabourReturn\Models\OutsideLabourReturnItem;
...
class Edit extends Component
{
    ...
    public string $status = OutsideLabourReturn::STATUS_REQUESTED;
    ...
    public function mount(?OutsideLabourReturn $outsideLabourReturn = null): void
    {
        if ($outsideLabourReturn && $outsideLabourReturn->exists) {
            $this->load($outsideLabourReturn);
    ...
    protected function load(OutsideLabourReturn $r): void
    {
        ...
        $this->items = $r->items->map(fn (OutsideLabourReturnItem $i) => [
            'id' => $i->id,
    ...
    {
        return [
            'return_type' => ['required', Rule::in(array_keys(OutsideLabourReturn::returnTypes()))],
            'claim_type' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::claimTypes()))],
            ...
            'return_reason' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::returnReasons()))],
            'warranty_type' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::warrantyTypes()))],
            'warranty_period' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::warrantyPeriods()))],
            'rework_type' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::reworkTypes()))],
            'tat_option' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::tatOptions()))],
            'tat_custom_days' => ['nullable', 'integer', 'min:1', 'max:365', Rule::requiredIf(fn () => $this->tat_option === 'custom')],
            'status' => ['required', Rule::in(array_keys(OutsideLabourReturn::statuses()))],
            'counter_proposal' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::counterProposals())), Rule::requiredIf(fn () => $this->status === OutsideLabourReturn::STATUS_COUNTER_PROPOSAL)],
            'rejection_reason' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::rejectionReasons())), Rule::requiredIf(fn () => $this->status === OutsideLabourReturn::STATUS_REJECTED)],
            'vendor_rating_type' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::vendorRatingTypes()))],
            'recovery_amount' => ['nullable', 'numeric', 'min:0'],
            'reminder_frequency' => ['nullable', Rule::in(array_keys(OutsideLabourReturn::reminderFrequencies()))],
            ...
            'items.*.item_type' => ['required', Rule::in(array_keys(OutsideLabourReturnItem::itemTypes()))],
            'items.*.description' => ['required', 'string', 'max:255'],
            'items.*.quantity' => ['required', 'numeric', 'min:0.01'],
            'items.*.rate' => ['nullable', 'numeric', 'min:0'],
            'items.*.material_condition' => ['nullable', Rule::in(array_keys(OutsideLabourReturnItem::materialConditions()))],
            'items.*.notes' => ['nullable', 'string', 'max:255'],
            'beforeFiles.*' => ['nullable', 'image', 'max:8192'],
            'afterFiles.*' => ['nullable', 'image', 'max:8192'],

            'attachments' => ['array'],
            'attachments.*.attachment_type' => ['nullable', Rule::in(array_keys(OutsideLabourReturnAttachment::attachmentTypes()))],
    ...
    {
        $this->authorize($this->editingId ? 'outside_labour_return.update' : 'outside_labour_return.create');
        ...
        $return = DB::transaction(function () use ($data, $items, $attachments, $isCreate) {
            if ($isCreate) {
                $row = OutsideLabourReturn::create($data);
                $this->editingId = $row->id;
                $this->return_no = $row->fresh()->return_no;
            } else {
                $row = OutsideLabourReturn::findOrFail($this->editingId);
                $row->update($data);
            }
        ...
        return redirect()->route('outside-labour-return.index');
    }

    /** @param  array<int, array<string, mixed>>  $rows */
    protected function syncItems(OutsideLabourReturn $return, array $rows): void
    {
        $keptIds = [];
        ...
        foreach (array_values($rows) as $i => $row) {
            ...
            if ($before instanceof TemporaryUploadedFile) {
                $beforePath = $before->store('outside-labour-returns/'.$return->id, 'public');
            }
            $after = $this->afterFiles[$i] ?? null;
            if ($after instanceof TemporaryUploadedFile) {
                $afterPath = $after->store('outside-labour-returns/'.$return->id, 'public');
            }
    ...
    /** @param  array<int, array<string, mixed>>  $rows */
    protected function syncAttachments(OutsideLabourReturn $return, array $rows): void
    {
        $keptIds = [];
        ...
        foreach (array_values($rows) as $i => $row) {
            ...
            if ($upload instanceof TemporaryUploadedFile) {
                $path = $upload->store('outside-labour-returns/'.$return->id, 'public');
    ...
    {
        return view('outside-labour-return::edit');
    }
}
