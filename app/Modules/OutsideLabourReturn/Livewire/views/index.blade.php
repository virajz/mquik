<div>
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Outside Labour Return / Warranty</flux:heading>
            <flux:text class="mt-1">Parts & labour warranty claims to outside vendors — rework, replacement, counter-proposal or settlement.</flux:text>
        </div>
        <div class="flex items-center gap-2">
            @can('outside_labour_return.view')
                <flux:button variant="ghost" icon="arrow-down-tray" wire:click="download">Return Register</flux:button>
            @endcan
            @can('outside_labour_return.create')
                <flux:button variant="primary" icon="plus" :href="route('outside-labour-return.create')" wire:navigate>New Return / Claim</flux:button>
            @endcan
        </div>
    </div>
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('outside_labour_return.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square" :href="route('outside-labour-return.edit', $row)" wire:navigate>Edit</flux:button>
                            @endcan
                            @can('outside_labour_return.delete')
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No returns / claims yet</div>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
