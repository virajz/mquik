<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Parking Slots</flux:heading>
            <flux:text class="mt-1">Where a vehicle waits on site — numbered slots plus outside-the-gate parking.</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('parking_slot_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('parking_slot_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'ParkingSlotMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('parking_slot_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'ParkingSlotMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'ParkingSlotMaster'" wire:key="export-parking-slots" />
    <livewire:import-export.import-wizard :module="'ParkingSlotMaster'" wire:key="import-parking-slots" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('parking_slot_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('parking_slot_master.delete')
                                <flux:modal.trigger :name="'parking-slot-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'parking-slot-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any appointments are using this parking slot the delete will fail and you'll see a warning.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('parking-slot-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No parking slots yet</div>
    ...
    <livewire:parking-slot-master.form />
</div>
