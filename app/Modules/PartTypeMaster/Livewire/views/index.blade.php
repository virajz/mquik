<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Part Types</flux:heading>
            <flux:text class="mt-1">Part sourcing types — Genuine, After Market, OEM, Refurbished.</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('part_type_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('part_type_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'PartTypeMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('part_type_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'PartTypeMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'PartTypeMaster'" wire:key="export-part-types" />
    <livewire:import-export.import-wizard :module="'PartTypeMaster'" wire:key="import-part-types" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('part_type_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('part_type_master.delete')
                                <flux:modal.trigger :name="'part-type-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'part-type-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('part-type-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No Part Types yet</div>
                        <flux:text class="mt-1">Add types like Genuine, After Market.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:part-type-master.form />
</div>
