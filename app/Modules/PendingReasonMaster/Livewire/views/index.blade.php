<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Pending Reasons</flux:heading>
            <flux:text class="mt-1">Why an appointment or pickup/drop job is still pending, or was rescheduled — shared across modules.</flux:text>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('pending_reason_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'PendingReasonMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('pending_reason_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'PendingReasonMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'PendingReasonMaster'" wire:key="export-pending-reasons" />
    <livewire:import-export.import-wizard :module="'PendingReasonMaster'" wire:key="import-pending-reasons" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('pending_reason_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('pending_reason_master.delete')
                                <flux:modal.trigger :name="'pending-reason-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'pending-reason-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any appointments are using this pending reason the delete will fail and you'll see a warning.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('pending-reason-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
    ...
    <livewire:pending-reason-master.form />
</div>
