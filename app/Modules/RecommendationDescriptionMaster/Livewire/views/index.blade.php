<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Recommendation Descriptions</flux:heading>
            <flux:text class="mt-1">The wording a technician picks on an inspection checkpoint, filed by category.</flux:text>
        </div>
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    <flux:menu.item icon="arrow-up-tray"
                        wire:click="$dispatch('start-import', { module: 'RecommendationDescriptionMaster' })">
                        Import…
                    </flux:menu.item>
                    <flux:menu.item icon="arrow-down-tray"
                        wire:click="$dispatch('start-export', { module: 'RecommendationDescriptionMaster' })">
                        Export
                    </flux:menu.item>
                </flux:menu>
            </flux:dropdown>
    ...
    <livewire:import-export.export-button :module="'RecommendationDescriptionMaster'" wire:key="export-recommendation-description-master" />
    <livewire:import-export.import-wizard :module="'RecommendationDescriptionMaster'" wire:key="import-recommendation-description-master" />
    ...
    <flux:table>
        <flux:table.columns>
            ...
            <flux:table.column class="w-40">Sub category</flux:table.column>
        ...
        <flux:table.rows>
            @forelse ($rows as $row)
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell class="text-zinc-500 text-sm">{{ $row->category?->name ?? '—' }}</flux:table.cell>
                    <flux:table.cell class="text-zinc-500 text-sm">{{ $row->subCategory?->name ?? '—' }}</flux:table.cell>
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            ...
                            <flux:modal.trigger :name="'recommendation-description-master-delete-' . $row->id">
                                <flux:button size="sm" variant="ghost" icon="trash" />
                            </flux:modal.trigger>

                            <flux:modal :name="'recommendation-description-master-delete-' . $row->id">
                                <div class="space-y-4">
                                    ...
                                    <div class="flex gap-2 justify-end">
                                        ...
                                        <flux:button variant="danger"
                                            ...
                                            x-on:click="$flux.modal('recommendation-description-master-delete-{{ $row->id }}').close()">
                                            Delete
                                        </flux:button>
                                    </div>
                                </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="8" class="text-center text-zinc-500 py-12">
                        ...
                        <flux:text class="mt-1">Add the phrases your technicians reach for most.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:recommendation-description-master.form />
</div>
