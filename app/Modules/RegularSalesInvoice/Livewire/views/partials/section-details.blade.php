...
<section class="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6 lg:gap-10">
    <div>
        ...
        <flux:text size="sm" class="mt-1 text-zinc-500">Customer, vehicle, references, packages, loss classification and warranty.</flux:text>
    </div>
    <div class="space-y-4 min-w-0">
        <div class="grid grid-cols-1 md:grid-cols-[1fr_200px] gap-4">
            ...
            <flux:select wire:model="invoice_type" variant="listbox" label="Invoice Type" required>
                @foreach (RegularSalesInvoice::invoiceTypes() as $k => $l)
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <flux:select wire:model="proforma_id" variant="listbox" searchable clearable label="Proforma" placeholder="Link…">
                @foreach ($this->proformas as $p)
                    <flux:select.option :value="$p->id" wire:key="pf-{{ $p->id }}">{{ $p->proforma_no }}</flux:select.option>
                @endforeach
            </flux:select>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <flux:select wire:model="warranty_type" variant="listbox" clearable label="Warranty Type" placeholder="—">
                @foreach (RegularSalesInvoice::warrantyTypes() as $k => $l)
                    <flux:select.option :value="$k">{{ $l }}</flux:select.option>
                @endforeach
            </flux:select>
            <flux:select wire:model="warranty_period" variant="listbox" clearable label="Warranty Period" placeholder="—">
                @foreach (RegularSalesInvoice::warrantyPeriods() as $k => $l)
                    <flux:select.option :value="$k">{{ $l }}</flux:select.option>
                @endforeach
            </flux:select>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <flux:select wire:model="vendor_id" variant="listbox" searchable clearable label="Vendor" placeholder="Contractor / outsource…" :filter="false">
            <x-slot name="search">
        ...
        <flux:textarea wire:model="recommended_service" label="Service Recommendation" rows="2" placeholder="Recommended next service / additional work for the customer." />
    </div>
</section>
