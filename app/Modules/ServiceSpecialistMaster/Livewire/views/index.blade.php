<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Service Specialists</flux:heading>
            <flux:text class="mt-1">Vendor service specialities (Denting, Painting, AC, Engine...).</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('service_specialist_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('service_specialist_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'ServiceSpecialistMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('service_specialist_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'ServiceSpecialistMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'ServiceSpecialistMaster'" wire:key="export-service-specialists" />
    <livewire:import-export.import-wizard :module="'ServiceSpecialistMaster'" wire:key="import-service-specialists" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('service_specialist_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('service_specialist_master.delete')
                                <flux:modal.trigger :name="'service-specialist-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'service-specialist-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('service-specialist-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No Service Specialists yet</div>
                        <flux:text class="mt-1">Add specialities like Denting, Painting, AC.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:service-specialist-master.form />
</div>
