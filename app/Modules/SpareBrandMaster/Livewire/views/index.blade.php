<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Spare Brands</flux:heading>
            <flux:text class="mt-1">Brands used across spares, inquiries, and procurement.</flux:text>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('spare_brand_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'SpareBrandMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('spare_brand_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'SpareBrandMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('spare_brand_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('spare_brand_master.delete')
                                <flux:modal.trigger :name="'spare-brand-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'spare-brand-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any spares are using this brand the delete will fail and you'll see a warning.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('spare-brand-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No brands yet</div>
                        <flux:text class="mt-1">Add brands like Bosch, Denso, NGK, MRF, Apollo.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:spare-brand-master.form />
</div>
