<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Taxes</flux:heading>
            <flux:text class="mt-1">GST/HSN tax codes used by spares, labour, and invoices.</flux:text>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('tax_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'TaxMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('tax_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'TaxMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'TaxMaster'" wire:key="export-taxes" />
    <livewire:import-export.import-wizard :module="'TaxMaster'" wire:key="import-taxes" />
    ...
    <flux:table>
        <flux:table.columns>
            ...
            <flux:table.column class="w-28" sortable :sorted="$sortBy === 'hsn_sac'" :direction="$sortDirection" wire:click="sort('hsn_sac')">
                HSN/SAC
            </flux:table.column>
            ...
            <flux:table.column class="w-24" align="end">
                Cess %
            </flux:table.column>
            ...
            @forelse ($rows as $row)
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell class="text-right font-mono">
                        {{ number_format((float) $row->gst_percent, 2) }}%
                    </flux:table.cell>

                    <flux:table.cell class="text-right font-mono text-zinc-500">
                        @if ((float) $row->cess_percent > 0)
                            {{ number_format((float) $row->cess_percent, 2) }}%
                        @else
                            —
                        @endif
                    </flux:table.cell>
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('tax_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('tax_master.delete')
                                <flux:modal.trigger :name="'tax-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'tax-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone. If any spares, services, or invoices reference this tax the delete will fail and you'll see a warning.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('tax-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="8" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No taxes yet</div>
                        <flux:text class="mt-1">Add tax codes like GST 5%, GST 12%, GST 18%, GST 28%.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
