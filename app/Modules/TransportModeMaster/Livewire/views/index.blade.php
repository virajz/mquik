<div>
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Transport Modes</flux:heading>
            <flux:text class="mt-1">How material is moved (Self, Courier, Porter, Liftor...).</flux:text>
        </div>

        <div class="flex items-center gap-2">
            @can('transport_mode_master.create')
            ...
            <flux:dropdown align="end">
                ...
                <flux:menu>
                    @can('transport_mode_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'TransportModeMaster' })">
                        Import…
                    </flux:menu.item>
                    @endcan
                    @can('transport_mode_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'TransportModeMaster' })">
                        Export
                    </flux:menu.item>
                    @endcan
                </flux:menu>
    ...
    <livewire:import-export.export-button :module="'TransportModeMaster'" wire:key="export-transport-modes" />
    <livewire:import-export.import-wizard :module="'TransportModeMaster'" wire:key="import-transport-modes" />
                ...
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('transport_mode_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square"
                                wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('transport_mode_master.delete')
                                <flux:modal.trigger :name="'transport-mode-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'transport-mode-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <flux:button variant="danger"
                                                ...
                                                x-on:click="$flux.modal('transport-mode-master-delete-{{ $row->id }}').close()">
                                                Delete
                                            </flux:button>
                                        </div>
                                    </div>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center text-zinc-500 py-12">
                        ...
                        <div class="font-medium">No Transport Modes yet</div>
                        <flux:text class="mt-1">Add modes like Self, Courier, Porter.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <livewire:transport-mode-master.form />
</div>
