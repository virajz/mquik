<div>
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">Vehicle Colors</flux:heading>
            <flux:text class="mt-1">Paint colors used on customer vehicles.</flux:text>
        </div>
        <div class="flex items-center gap-2">
            @can('vehicle_color_master.create')
                <flux:button variant="primary" icon="plus" wire:click="openCreate">New Color</flux:button>
            @endcan
            <flux:dropdown align="end">
                <flux:button variant="ghost" icon="ellipsis-vertical" />
                <flux:menu>
                    @can('vehicle_color_master.import')
                    <flux:menu.item icon="arrow-up-tray" wire:click="$dispatch('start-import', { module: 'VehicleColorMaster' })">Import…</flux:menu.item>
                    @endcan
                    @can('vehicle_color_master.export')
                    <flux:menu.item icon="arrow-down-tray" wire:click="$dispatch('start-export', { module: 'VehicleColorMaster' })">Export</flux:menu.item>
                    @endcan
                </flux:menu>
            </flux:dropdown>
        </div>
        ...
        <flux:table.rows>
            @forelse ($rows as $row)
                <flux:table.row :key="$row->id">
                    ...
                    <flux:table.cell>
                        <span class="block size-6 rounded-full border border-zinc-200 dark:border-zinc-700" style="background-color: {{ $row->hex_code ?: '#ffffff' }}"></span>
                    </flux:table.cell>
                    ...
                    <flux:table.cell>
                        <div class="flex items-center justify-end gap-1">
                            @can('vehicle_color_master.update')
                                <flux:button size="sm" variant="ghost" icon="pencil-square" wire:click="openEdit({{ $row->id }})">Edit</flux:button>
                            @endcan
                            @can('vehicle_color_master.delete')
                                <flux:modal.trigger :name="'vehicle-color-master-delete-' . $row->id">
                                    <flux:button size="sm" variant="ghost" icon="trash" />
                                </flux:modal.trigger>
                                <flux:modal :name="'vehicle-color-master-delete-' . $row->id">
                                    <div class="space-y-4">
                                        <flux:heading size="lg">Delete {{ $row->name }}?</flux:heading>
                                        <flux:text>Cannot be undone.</flux:text>
                                        <div class="flex gap-2 justify-end">
                                            <flux:modal.close><flux:button variant="ghost">Cancel</flux:button></flux:modal.close>
                                            <flux:button variant="danger" wire:click="delete({{ $row->id }})" x-on:click="$flux.modal('vehicle-color-master-delete-{{ $row->id }}').close()">Delete</flux:button>
                                        </div>
                                    </div>
                                </flux:modal>
                            @endcan
    ...
    <livewire:vehicle-color-master.form />
    <livewire:import-export.export-button :module="'VehicleColorMaster'" wire:key="export-vehicle-color-master" />
    <livewire:import-export.import-wizard :module="'VehicleColorMaster'" wire:key="import-vehicle-color-master" />
</div>
