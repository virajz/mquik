...
return new class extends Migration
{
    ...
    {
        Schema::create('vehicle_inspection_orders', function (Blueprint $table) {
            $table->id();
        ...
        Schema::create('vehicle_inspection_order_items', function (Blueprint $table) {
            $table->id();
            $table->foreignId('vehicle_inspection_order_id')->constrained('vehicle_inspection_orders')->cascadeOnDelete();
            ...
            $table->index(['vehicle_inspection_order_id', 'sequence_no']);
            $table->index(['vehicle_inspection_order_id', 'result']);
        });

        Schema::create('vehicle_inspection_order_pauses', function (Blueprint $table) {
            $table->id();
            $table->foreignId('vehicle_inspection_order_id')->constrained('vehicle_inspection_orders')->cascadeOnDelete();
            $table->foreignId('hold_reason_id')->nullable()->constrained('work_order_hold_reasons')->nullOnDelete();
            ...
            $table->index(['vehicle_inspection_order_id', 'paused_at']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('vehicle_inspection_order_pauses');
        Schema::dropIfExists('vehicle_inspection_order_items');
        Schema::dropIfExists('vehicle_inspection_orders');
    }
};
