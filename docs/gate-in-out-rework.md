...
- [x] 3. Show vehicle name (brand/model) for the selected reg. no.
- [x] 4. Entry Gate defaults to GATE NO. 1
- [x] 5. Exit Gate defaults to GATE NO. 2
- [x] 8. Delivered By — only advisors and cashiers
- [x] 9. Rename "Exit Approved By" → "Exit By (Security Guard)", only security guards offered

## Chunk 2 — Delivery flow & status
- [x] 10. Exit Date & Exit Time — disabled; stamped by a "Mark delivered" action
- [x] 11. Status auto-derived (pending → completed on delivery; cancel stays deliberate)
- [x] 12. Mandatory: entry set on create (date/time/reg/gate/source); exit set (gate, driver type, delivered by, exit by) enforced at delivery
- [x] 13. Image capture option (`captured_image_path` column exists, no UI yet)

## Chunk 3 — History (listing)
- [x] 14. Default to Pending (vehicles still un-delivered) on load
- [x] 15. Outward Date From & To filter
- [x] 16. New columns: Vehicle (brand/model), Job Card No., Delivered By, Driver Type, Exit By
- [x] 17. Sort on all column headings
...
- **Duplicate module** — `VehicleMovement` ("Vehicle Inward / Outward") vs GateInOut: confirmed duplication, 1 row of data, but it carries `gate_pass_approval_id` + attachments that GateInOut lacks. **Done:** menu entry and routes retired; models/table/permissions kept until the gate-pass link is ported.
