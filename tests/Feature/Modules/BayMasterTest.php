...
use App\Modules\BayMaster\Livewire\Form;
...
it('renders the index page', function () {
    BayMaster::factory()->count(3)->create();

    $this->get(route('bay-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    BayMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    BayMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    BayMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    BayMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    BayMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    BayMaster::factory()->create(['name' => 'ALPHA TYPE']);
    BayMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('bay-master:saved');

    $record = BayMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = BayMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('bay-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = BayMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(BayMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    BayMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = BayMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('bay-master.index'))->assertRedirect(route('login'));
});
