...
use App\Modules\ChallanReasonMaster\Livewire\Form;
...
it('renders the index page', function () {
    ChallanReasonMaster::factory()->count(3)->create();

    $this->get(route('challan-reason-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    ChallanReasonMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    ChallanReasonMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    ChallanReasonMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    ChallanReasonMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    ChallanReasonMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    ChallanReasonMaster::factory()->create(['name' => 'ALPHA TYPE']);
    ChallanReasonMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('challan-reason-master:saved');

    $record = ChallanReasonMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = ChallanReasonMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('challan-reason-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = ChallanReasonMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(ChallanReasonMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    ChallanReasonMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = ChallanReasonMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('challan-reason-master.index'))->assertRedirect(route('login'));
});
