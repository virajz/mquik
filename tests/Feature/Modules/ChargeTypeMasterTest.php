...
use App\Modules\ChargeTypeMaster\Livewire\Form;
...
it('renders the index page', function () {
    ChargeTypeMaster::factory()->count(3)->create();

    $this->get(route('charge-type-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    ChargeTypeMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    ChargeTypeMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    ChargeTypeMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    ChargeTypeMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    ChargeTypeMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    ChargeTypeMaster::factory()->create(['name' => 'ALPHA TYPE']);
    ChargeTypeMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('charge-type-master:saved');

    $record = ChargeTypeMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = ChargeTypeMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('charge-type-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = ChargeTypeMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(ChargeTypeMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    ChargeTypeMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = ChargeTypeMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('charge-type-master.index'))->assertRedirect(route('login'));
});
