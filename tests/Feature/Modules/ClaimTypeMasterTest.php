...
use App\Modules\ClaimTypeMaster\Livewire\Form;
...
it('renders the index page', function () {
    ClaimTypeMaster::factory()->count(3)->create();

    $this->get(route('claim-type-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    ClaimTypeMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    ClaimTypeMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    ClaimTypeMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    ClaimTypeMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    ClaimTypeMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    ClaimTypeMaster::factory()->create(['name' => 'ALPHA TYPE']);
    ClaimTypeMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('claim-type-master:saved');

    $record = ClaimTypeMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = ClaimTypeMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('claim-type-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = ClaimTypeMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(ClaimTypeMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    ClaimTypeMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = ClaimTypeMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('claim-type-master.index'))->assertRedirect(route('login'));
});
