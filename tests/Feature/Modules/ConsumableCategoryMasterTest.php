...
use App\Modules\ConsumableCategoryMaster\Livewire\Form;
...
it('renders the index page', function () {
    ConsumableCategoryMaster::factory()->count(3)->create();

    $this->get(route('consumable-category-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    ConsumableCategoryMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    ConsumableCategoryMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    ConsumableCategoryMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    ConsumableCategoryMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    ConsumableCategoryMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    ConsumableCategoryMaster::factory()->create(['name' => 'ALPHA TYPE']);
    ConsumableCategoryMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('consumable-category-master:saved');

    $record = ConsumableCategoryMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = ConsumableCategoryMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('consumable-category-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = ConsumableCategoryMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(ConsumableCategoryMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    ConsumableCategoryMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = ConsumableCategoryMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('consumable-category-master.index'))->assertRedirect(route('login'));
});
