...
use App\Modules\DamageCauseMaster\Livewire\Form;
...
it('renders the index page', function () {
    DamageCauseMaster::factory()->count(3)->create();

    $this->get(route('damage-cause-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    DamageCauseMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    DamageCauseMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    DamageCauseMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    DamageCauseMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    DamageCauseMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    DamageCauseMaster::factory()->create(['name' => 'ALPHA TYPE']);
    DamageCauseMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('damage-cause-master:saved');

    $record = DamageCauseMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = DamageCauseMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('damage-cause-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = DamageCauseMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(DamageCauseMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    DamageCauseMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = DamageCauseMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('damage-cause-master.index'))->assertRedirect(route('login'));
});
