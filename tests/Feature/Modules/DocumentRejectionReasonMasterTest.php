...
use App\Modules\DocumentRejectionReasonMaster\Livewire\Form;
...
it('renders the index page', function () {
    DocumentRejectionReasonMaster::factory()->count(3)->create();

    $this->get(route('document-rejection-reason-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    DocumentRejectionReasonMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    DocumentRejectionReasonMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    DocumentRejectionReasonMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    DocumentRejectionReasonMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    DocumentRejectionReasonMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    DocumentRejectionReasonMaster::factory()->create(['name' => 'ALPHA TYPE']);
    DocumentRejectionReasonMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('document-rejection-reason-master:saved');

    $record = DocumentRejectionReasonMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = DocumentRejectionReasonMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('document-rejection-reason-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = DocumentRejectionReasonMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(DocumentRejectionReasonMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    DocumentRejectionReasonMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = DocumentRejectionReasonMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('document-rejection-reason-master.index'))->assertRedirect(route('login'));
});
