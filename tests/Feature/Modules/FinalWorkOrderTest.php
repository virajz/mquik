...
it('renders the index page', function () {
    FinalWorkOrder::factory()->count(2)->create();

    $this->get(route('final-work-order.index'))
        ->assertOk()
...
it('requires authentication', function () {
    auth()->logout();
    $this->get(route('final-work-order.index'))->assertRedirect(route('login'));
});
...
it('creates a work order, stamps VIO number, and redirects into the editor', function () {
    ...
    $order = FinalWorkOrder::firstOrFail();
    expect($order->order_no)->toBe('FWO-'.str_pad((string) $order->id, 5, '0', STR_PAD_LEFT))
        ->and($order->priority->name)->toBe('HIGH')
        ->and($order->status)->toBe(FinalWorkOrder::STATUS_ASSIGNMENT_PENDING);
});
...
it('saves checklist items with results and before/after photos', function () {
    Storage::fake('public');
    $order = FinalWorkOrder::factory()->create();

    Livewire::test(Edit::class, ['finalWorkOrder' => $order])
        ->set('items', [
...
it('logs a work order started history event on the job card when status moves to wip', function () {
    $jobCard = JobCard::factory()->create();
    $order = FinalWorkOrder::factory()->create(['job_card_id' => $jobCard->id]);

    Livewire::test(Edit::class, ['finalWorkOrder' => $order])
        ->set('status', FinalWorkOrder::STATUS_WIP)
        ->call('save')
        ->assertHasNoErrors();
...
it('saves the pause/resume log', function () {
    $order = FinalWorkOrder::factory()->wip()->create();
...
it('deletes a work order from the index', function () {
    ...
    expect(FinalWorkOrder::find($order->id))->toBeNull();
});
...
it('counts pending, active, completed and cancelled inspections', function () {
    FinalWorkOrder::factory()->create(['status' => FinalWorkOrder::STATUS_ASSIGNMENT_PENDING]);
    FinalWorkOrder::factory()->create(['status' => FinalWorkOrder::STATUS_ASSIGNED]);
    FinalWorkOrder::factory()->create(['status' => FinalWorkOrder::STATUS_WIP]);
    FinalWorkOrder::factory()->create(['status' => FinalWorkOrder::STATUS_ON_HOLD]);
    FinalWorkOrder::factory()->create(['status' => FinalWorkOrder::STATUS_COMPLETED]);
    FinalWorkOrder::factory()->create(['status' => FinalWorkOrder::STATUS_CANCELLED]);
...
it('persists item hours and computes net TAT', function () {
    $fwo = FinalWorkOrder::factory()->create();

    Livewire::test(Edit::class, ['finalWorkOrder' => $fwo])
        ->call('addItem')
