...
use App\Modules\FuelTypeMaster\Livewire\Form;
...
it('renders the index page', function () {
    FuelTypeMaster::factory()->count(3)->create();

    $this->get(route('fuel-type-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    FuelTypeMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    FuelTypeMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    FuelTypeMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    FuelTypeMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    FuelTypeMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    FuelTypeMaster::factory()->create(['name' => 'ALPHA TYPE']);
    FuelTypeMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('fuel-type-master:saved');

    $record = FuelTypeMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = FuelTypeMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('fuel-type-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = FuelTypeMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(FuelTypeMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    FuelTypeMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = FuelTypeMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('fuel-type-master.index'))->assertRedirect(route('login'));
});
