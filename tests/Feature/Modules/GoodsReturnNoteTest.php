...
it('renders the index page', function () {
    GoodsReturnNote::factory()->count(3)->create();

    $this->get(route('goods-return-note.index'))->assertOk()->assertSeeLivewire(Index::class);
});

it('renders the create page', function () {
    $this->get(route('goods-return-note.create'))->assertOk()->assertSeeLivewire(Edit::class);
});
...
it('creates a return with a part line and a labour line and redirects', function () {
    ...
    Livewire::test(Edit::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertRedirect(route('goods-return-note.index'));

    $r = GoodsReturnNote::with('items')->first();
    expect($r->return_no)->toBe('GRTN-'.str_pad((string) $r->id, 5, '0', STR_PAD_LEFT))
...
it('requires a rejection reason when rejected', function () {
    ...
    Livewire::test(Edit::class)
        ...
        ->set('status', GoodsReturnNote::STATUS_REJECTED)
        ->call('save')
...
it('requires a counter proposal when status is counter proposal', function () {
    ...
    Livewire::test(Edit::class)
        ->set('vendor_id', $vendor->id)
        ->set('items.0.description', 'part')
        ->set('status', GoodsReturnNote::STATUS_COUNTER_PROPOSAL)
        ->call('save')
...
it('reports KPI counts on the index', function () {
    GoodsReturnNote::factory()->count(2)->create(['recovery_amount' => 1000]); // open
    GoodsReturnNote::factory()->accepted()->create(['recovery_amount' => 5000]); // closed
    GoodsReturnNote::factory()->rejected()->create(); // closed
...
it('deletes a return', function () {
    $r = GoodsReturnNote::factory()->create();

    Livewire::test(Index::class)->call('delete', $r->id);

    expect(GoodsReturnNote::find($r->id))->toBeNull();
});

it('downloads the return register as a CSV stream', function () {
    GoodsReturnNote::factory()->count(2)->create();
