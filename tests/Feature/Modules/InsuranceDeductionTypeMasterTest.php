...
use App\Modules\InsuranceDeductionTypeMaster\Livewire\Form;
...
it('renders the index page', function () {
    InsuranceDeductionTypeMaster::factory()->count(3)->create();

    $this->get(route('insurance-deduction-type-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    InsuranceDeductionTypeMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    InsuranceDeductionTypeMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    InsuranceDeductionTypeMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    InsuranceDeductionTypeMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    InsuranceDeductionTypeMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    InsuranceDeductionTypeMaster::factory()->create(['name' => 'ALPHA TYPE']);
    InsuranceDeductionTypeMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('insurance-deduction-type-master:saved');

    $record = InsuranceDeductionTypeMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = InsuranceDeductionTypeMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('insurance-deduction-type-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = InsuranceDeductionTypeMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(InsuranceDeductionTypeMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    InsuranceDeductionTypeMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = InsuranceDeductionTypeMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('insurance-deduction-type-master.index'))->assertRedirect(route('login'));
});
