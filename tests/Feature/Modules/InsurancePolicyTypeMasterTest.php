...
use App\Modules\InsurancePolicyTypeMaster\Livewire\Form;
...
it('renders the index page', function () {
    InsurancePolicyTypeMaster::factory()->count(3)->create();

    $this->get(route('insurance-policy-type-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    InsurancePolicyTypeMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    InsurancePolicyTypeMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    InsurancePolicyTypeMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    InsurancePolicyTypeMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    InsurancePolicyTypeMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    InsurancePolicyTypeMaster::factory()->create(['name' => 'ALPHA TYPE']);
    InsurancePolicyTypeMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('insurance-policy-type-master:saved');

    $record = InsurancePolicyTypeMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = InsurancePolicyTypeMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('insurance-policy-type-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = InsurancePolicyTypeMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(InsurancePolicyTypeMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    InsurancePolicyTypeMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = InsurancePolicyTypeMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('insurance-policy-type-master.index'))->assertRedirect(route('login'));
});
