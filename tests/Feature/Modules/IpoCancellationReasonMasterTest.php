...
use App\Modules\IpoCancellationReasonMaster\Livewire\Form;
...
it('renders the index page', function () {
    IpoCancellationReasonMaster::factory()->count(3)->create();

    $this->get(route('ipo-cancellation-reason-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    IpoCancellationReasonMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    IpoCancellationReasonMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    IpoCancellationReasonMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    IpoCancellationReasonMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    IpoCancellationReasonMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    IpoCancellationReasonMaster::factory()->create(['name' => 'ALPHA TYPE']);
    IpoCancellationReasonMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('ipo-cancellation-reason-master:saved');

    $record = IpoCancellationReasonMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = IpoCancellationReasonMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('ipo-cancellation-reason-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = IpoCancellationReasonMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(IpoCancellationReasonMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    IpoCancellationReasonMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = IpoCancellationReasonMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('ipo-cancellation-reason-master.index'))->assertRedirect(route('login'));
});
