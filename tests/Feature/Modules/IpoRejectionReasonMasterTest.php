...
use App\Modules\IpoRejectionReasonMaster\Livewire\Form;
...
it('renders the index page', function () {
    IpoRejectionReasonMaster::factory()->count(3)->create();

    $this->get(route('ipo-rejection-reason-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    IpoRejectionReasonMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    IpoRejectionReasonMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    IpoRejectionReasonMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    IpoRejectionReasonMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    IpoRejectionReasonMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    IpoRejectionReasonMaster::factory()->create(['name' => 'ALPHA TYPE']);
    IpoRejectionReasonMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('ipo-rejection-reason-master:saved');

    $record = IpoRejectionReasonMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = IpoRejectionReasonMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('ipo-rejection-reason-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = IpoRejectionReasonMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(IpoRejectionReasonMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    IpoRejectionReasonMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = IpoRejectionReasonMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('ipo-rejection-reason-master.index'))->assertRedirect(route('login'));
});
