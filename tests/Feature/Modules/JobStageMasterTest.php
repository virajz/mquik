...
use App\Modules\JobStageMaster\Livewire\Form;
...
it('renders the index page', function () {
    JobStageMaster::factory()->count(3)->create();

    $this->get(route('job-stage-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    JobStageMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    JobStageMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    JobStageMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    JobStageMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    JobStageMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    JobStageMaster::factory()->create(['name' => 'ALPHA TYPE']);
    JobStageMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('job-stage-master:saved');

    $record = JobStageMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = JobStageMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('job-stage-master:edit', id: $record->id)
...
it('stores the track and sort order', function () {
    Livewire::test(Form::class)
        ->set('name', 'surveyor inspection')
        ->set('track', 'insurance')
        ->set('sort_order', 3)
        ->call('save')
        ->assertHasNoErrors();
    ...
    $record = JobStageMaster::firstOrFail();
    expect($record->track)->toBe('insurance')
        ->and($record->sort_order)->toBe(3);
});

it('deletes a type from the index', function () {
    $record = JobStageMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(JobStageMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    JobStageMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = JobStageMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('job-stage-master.index'))->assertRedirect(route('login'));
});
