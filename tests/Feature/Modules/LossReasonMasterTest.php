...
use App\Modules\LossReasonMaster\Livewire\Form;
...
it('renders the index page', function () {
    LossReasonMaster::factory()->count(3)->create();

    $this->get(route('loss-reason-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    LossReasonMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    LossReasonMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    LossReasonMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    LossReasonMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    LossReasonMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    LossReasonMaster::factory()->create(['name' => 'ALPHA TYPE']);
    LossReasonMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('loss-reason-master:saved');

    $record = LossReasonMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = LossReasonMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('loss-reason-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = LossReasonMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(LossReasonMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    LossReasonMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = LossReasonMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('loss-reason-master.index'))->assertRedirect(route('login'));
});
