...
use App\Modules\LossTypeMaster\Livewire\Form;
...
it('renders the index page', function () {
    LossTypeMaster::factory()->count(3)->create();

    $this->get(route('loss-type-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    LossTypeMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    LossTypeMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    LossTypeMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    LossTypeMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    LossTypeMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    LossTypeMaster::factory()->create(['name' => 'ALPHA TYPE']);
    LossTypeMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('loss-type-master:saved');

    $record = LossTypeMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = LossTypeMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('loss-type-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = LossTypeMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(LossTypeMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    LossTypeMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = LossTypeMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('loss-type-master.index'))->assertRedirect(route('login'));
});
