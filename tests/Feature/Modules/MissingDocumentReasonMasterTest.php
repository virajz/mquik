...
use App\Modules\MissingDocumentReasonMaster\Livewire\Form;
...
it('renders the index page', function () {
    MissingDocumentReasonMaster::factory()->count(3)->create();

    $this->get(route('missing-document-reason-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    MissingDocumentReasonMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    MissingDocumentReasonMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    MissingDocumentReasonMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    MissingDocumentReasonMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    MissingDocumentReasonMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    MissingDocumentReasonMaster::factory()->create(['name' => 'ALPHA TYPE']);
    MissingDocumentReasonMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('missing-document-reason-master:saved');

    $record = MissingDocumentReasonMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = MissingDocumentReasonMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('missing-document-reason-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = MissingDocumentReasonMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(MissingDocumentReasonMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    MissingDocumentReasonMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = MissingDocumentReasonMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('missing-document-reason-master.index'))->assertRedirect(route('login'));
});
