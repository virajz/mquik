...
it('renders the index page', function () {
    OutsideLabourOrder::factory()->count(2)->create();

    $this->get(route('outside-labour-order.index'))
        ->assertOk()
...
it('requires authentication', function () {
    auth()->logout();
    $this->get(route('outside-labour-order.index'))->assertRedirect(route('login'));
});
...
it('creates a work order, stamps VIO number, and redirects into the editor', function () {
    ...
    $order = OutsideLabourOrder::firstOrFail();
    expect($order->order_no)->toBe('OLO-'.str_pad((string) $order->id, 5, '0', STR_PAD_LEFT))
        ->and($order->priority->name)->toBe('HIGH')
        ->and($order->status)->toBe(OutsideLabourOrder::STATUS_ASSIGNMENT_PENDING);
});
...
it('saves checklist items with results and before/after photos', function () {
    Storage::fake('public');
    $order = OutsideLabourOrder::factory()->create();

    Livewire::test(Edit::class, ['outsideLabourOrder' => $order])
        ->set('items', [
...
it('logs a work order started history event on the job card when status moves to wip', function () {
    $jobCard = JobCard::factory()->create();
    $order = OutsideLabourOrder::factory()->create(['job_card_id' => $jobCard->id]);

    Livewire::test(Edit::class, ['outsideLabourOrder' => $order])
        ->set('status', OutsideLabourOrder::STATUS_WIP)
        ->call('save')
        ->assertHasNoErrors();
...
it('saves the pause/resume log', function () {
    $order = OutsideLabourOrder::factory()->wip()->create();
...
it('deletes a work order from the index', function () {
    ...
    expect(OutsideLabourOrder::find($order->id))->toBeNull();
});
...
it('surfaces technician findings as the order\'s additional work', function () {
    ...
    TechnicianFinding::factory()->create([
        'outside_labour_order_id' => $order->id,
...
it('counts pending, active, completed and cancelled inspections', function () {
    OutsideLabourOrder::factory()->create(['status' => OutsideLabourOrder::STATUS_ASSIGNMENT_PENDING]);
    OutsideLabourOrder::factory()->create(['status' => OutsideLabourOrder::STATUS_ASSIGNED]);
    OutsideLabourOrder::factory()->create(['status' => OutsideLabourOrder::STATUS_WIP]);
    OutsideLabourOrder::factory()->create(['status' => OutsideLabourOrder::STATUS_ON_HOLD]);
    OutsideLabourOrder::factory()->create(['status' => OutsideLabourOrder::STATUS_COMPLETED]);
    OutsideLabourOrder::factory()->create(['status' => OutsideLabourOrder::STATUS_CANCELLED]);
...
it('persists item hours and computes net TAT', function () {
    $fwo = OutsideLabourOrder::factory()->create();

    Livewire::test(Edit::class, ['outsideLabourOrder' => $fwo])
        ->call('addItem')
        ->set('items.0.label', 'brake job')
        ->set('items.0.result', 'ok')
        ->set('items.0.hours', 2.5)
        ->call('save')
        ->assertHasNoErrors();

    $fresh = $fwo->fresh()->load('items');
    expect((float) $fresh->items->first()->hours)->toBe(2.5)
        ->and($fresh->totalItemHours())->toBe(2.5);
});

it('exports the work order analysis CSV', function () {
...
it('saves the outside-labour fields (vendor, order type, communication)', function () {
    ...
    Livewire::test(Edit::class)
        ...
        ->set('order_type_id', $type->id)
        ->set('communication_mode', 'whatsapp')
        ->set('sequence_no', 3)
        ->call('save')
        ->assertHasNoErrors();

    $olo = OutsideLabourOrder::first();
    expect($olo->vendor_id)->toBe($vendor->id)
        ->and($olo->order_type_id)->toBe($type->id)
        ->and($olo->communication_mode)->toBe('whatsapp')
        ->and($olo->sequence_no)->toBe(3);
});
