...
use App\Modules\PendingReasonMaster\Livewire\Form;
...
it('renders the index page', function () {
    PendingReasonMaster::factory()->count(3)->create();

    $this->get(route('pending-reason-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    PendingReasonMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    PendingReasonMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    PendingReasonMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    PendingReasonMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    PendingReasonMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    PendingReasonMaster::factory()->create(['name' => 'ALPHA TYPE']);
    PendingReasonMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('pending-reason-master:saved');

    $record = PendingReasonMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = PendingReasonMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('pending-reason-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = PendingReasonMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(PendingReasonMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    PendingReasonMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = PendingReasonMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('pending-reason-master.index'))->assertRedirect(route('login'));
});
