...
use App\Modules\RegistrationTypeMaster\Livewire\Form;
...
it('renders the index page', function () {
    RegistrationTypeMaster::factory()->count(3)->create();

    $this->get(route('registration-type-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    RegistrationTypeMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    RegistrationTypeMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    RegistrationTypeMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    RegistrationTypeMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    RegistrationTypeMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    RegistrationTypeMaster::factory()->create(['name' => 'ALPHA TYPE']);
    RegistrationTypeMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('registration-type-master:saved');

    $record = RegistrationTypeMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = RegistrationTypeMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('registration-type-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = RegistrationTypeMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(RegistrationTypeMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    RegistrationTypeMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = RegistrationTypeMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('registration-type-master.index'))->assertRedirect(route('login'));
});
