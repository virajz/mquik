...
use App\Modules\RequestedRepairMaster\Livewire\Form;
...
it('renders the index page', function () {
    RequestedRepairMaster::factory()->count(3)->create();

    $this->get(route('requested-repair-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    RequestedRepairMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    RequestedRepairMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    RequestedRepairMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    RequestedRepairMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    RequestedRepairMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    RequestedRepairMaster::factory()->create(['name' => 'ALPHA TYPE']);
    RequestedRepairMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('requested-repair-master:saved');

    $record = RequestedRepairMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = RequestedRepairMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('requested-repair-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = RequestedRepairMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(RequestedRepairMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    RequestedRepairMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = RequestedRepairMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('requested-repair-master.index'))->assertRedirect(route('login'));
});
