...
use App\Modules\ReworkReasonMaster\Livewire\Form;
...
it('renders the index page', function () {
    ReworkReasonMaster::factory()->count(3)->create();

    $this->get(route('rework-reason-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    ReworkReasonMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    ReworkReasonMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    ReworkReasonMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    ReworkReasonMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    ReworkReasonMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    ReworkReasonMaster::factory()->create(['name' => 'ALPHA TYPE']);
    ReworkReasonMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('rework-reason-master:saved');

    $record = ReworkReasonMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = ReworkReasonMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('rework-reason-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = ReworkReasonMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(ReworkReasonMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    ReworkReasonMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = ReworkReasonMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('rework-reason-master.index'))->assertRedirect(route('login'));
});
