...
use App\Modules\StandardObservationMaster\Livewire\Form;
...
it('renders the index page', function () {
    StandardObservationMaster::factory()->count(3)->create();

    $this->get(route('standard-observation-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    StandardObservationMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    StandardObservationMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    StandardObservationMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    StandardObservationMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    StandardObservationMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    StandardObservationMaster::factory()->create(['name' => 'ALPHA TYPE']);
    StandardObservationMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('standard-observation-master:saved');

    $record = StandardObservationMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = StandardObservationMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('standard-observation-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = StandardObservationMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(StandardObservationMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    StandardObservationMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = StandardObservationMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('standard-observation-master.index'))->assertRedirect(route('login'));
});
