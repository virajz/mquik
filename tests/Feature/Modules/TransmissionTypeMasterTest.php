...
use App\Modules\TransmissionTypeMaster\Livewire\Form;
...
it('renders the index page', function () {
    TransmissionTypeMaster::factory()->count(3)->create();

    $this->get(route('transmission-type-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    TransmissionTypeMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    TransmissionTypeMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    TransmissionTypeMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    TransmissionTypeMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    TransmissionTypeMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    TransmissionTypeMaster::factory()->create(['name' => 'ALPHA TYPE']);
    TransmissionTypeMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('transmission-type-master:saved');

    $record = TransmissionTypeMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = TransmissionTypeMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('transmission-type-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = TransmissionTypeMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(TransmissionTypeMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    TransmissionTypeMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = TransmissionTypeMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('transmission-type-master.index'))->assertRedirect(route('login'));
});
