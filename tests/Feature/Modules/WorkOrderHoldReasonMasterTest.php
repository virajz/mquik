...
use App\Modules\WorkOrderHoldReasonMaster\Livewire\Form;
...
it('renders the index page', function () {
    WorkOrderHoldReasonMaster::factory()->count(3)->create();

    $this->get(route('work-order-hold-reason-master.index'))
        ->assertOk()
        ->assertSeeLivewire(Index::class);
});

it('searches by name or code', function () {
    WorkOrderHoldReasonMaster::factory()->create(['name' => 'ENGINEEE NOISE', 'code' => 'ENG']);
    WorkOrderHoldReasonMaster::factory()->create(['name' => 'BRAKEEE PROBLEM', 'code' => 'BRK']);
...
it('filters by active status', function () {
    WorkOrderHoldReasonMaster::factory()->create(['name' => 'ENGINE ENABLED']);
    WorkOrderHoldReasonMaster::factory()->inactive()->create(['name' => 'BRAKE DISABLED']);
...
it('sorts by name alphabetically by default', function () {
    WorkOrderHoldReasonMaster::factory()->create(['name' => 'ZEBRA TYPE']);
    WorkOrderHoldReasonMaster::factory()->create(['name' => 'ALPHA TYPE']);
    WorkOrderHoldReasonMaster::factory()->create(['name' => 'MIDDLE TYPE']);
...
it('creates a type with code', function () {
    Livewire::test(Form::class)
        ...
        ->call('save')
        ->assertHasNoErrors()
        ->assertDispatched('work-order-hold-reason-master:saved');

    $record = WorkOrderHoldReasonMaster::firstOrFail();
...
it('updates an existing type', function () {
    $record = WorkOrderHoldReasonMaster::factory()->create(['name' => 'OLD NAME']);

    Livewire::test(Form::class)
        ->dispatch('work-order-hold-reason-master:edit', id: $record->id)
...
it('deletes a type from the index', function () {
    $record = WorkOrderHoldReasonMaster::factory()->create();

    Livewire::test(Index::class)->call('delete', $record->id);

    expect(WorkOrderHoldReasonMaster::find($record->id))->toBeNull();
});

it('validates name is required and unique', function () {
    WorkOrderHoldReasonMaster::factory()->create(['name' => 'EXISTING']);
...
it('allows updating a type without triggering self-uniqueness conflict', function () {
    $record = WorkOrderHoldReasonMaster::factory()->create(['name' => 'ENGINE NOISE']);
...
it('requires authentication', function () {
    ...
    $this->get(route('work-order-hold-reason-master.index'))->assertRedirect(route('login'));
});
